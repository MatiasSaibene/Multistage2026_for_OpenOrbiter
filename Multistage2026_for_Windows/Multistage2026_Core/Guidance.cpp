...
void Multistage2026::Attitude(double pitch,double roll, double heading,double pitchrate=2, double rollrate=8, double yawrate=5){
	...
	//	if(oapiGetTimeAcceleration()>1){
		...
		if(abs(pDelta)>pdb){
				...
				tgtrotlevel.x=min(abs(ReqTorque.x/MaxTorque.x),1.0)*SignOf(ReqTorque.x);
	...
	if(abs(rDelta)>pdb){
				...
				tgtrotlevel.z=min(abs(ReqTorque.z/MaxTorque.z),1.0)*SignOf(ReqTorque.z);
		...
		if(YawCtrl){
			...
			tgtrotlevel.y=min(abs(ReqTorque.y/MaxTorque.y),1.0)*SignOf(ReqTorque.y);
			} else {
				tgtrotlevel.y=0;
			}
...
void Multistage2026::VinkaComposeGNCSteps(){
    ...
    for(int i = 0; i <= nsteps; i++){
        ...
        case GNC_Comand::CM_ORBIT:
            ...
            oapiWriteLog(const_cast<char *>("Orbit Call Found!"));
            break;
...
void Multistage2026::WriteGNCFile(){
	...
	GncFile = oapiOpenFile(filenmbuff.string().c_str(),FILE_OUT,CONFIG);
