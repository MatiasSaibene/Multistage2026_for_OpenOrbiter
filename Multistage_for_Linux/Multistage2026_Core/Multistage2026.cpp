...
double Multistage2026::RemBurnTime(int rbtstage, double level = 1){
    ...
    if (level <= 0.001 || stage->at(rbtstage).thrust <= 0) {
        return 0.0;
    }

    double m_prop = GetPropellantMass(stage->at(rbtstage).tank);
    if (m_prop <= 0) return 0.0;

    double thrust_active = stage->at(rbtstage).thrust * level;
    double BT = (stage->at(rbtstage).isp * m_prop * G) / thrust_active;
...
double Multistage2026::BoosterRemBurnTime(int rbtbooster, double level = 1) {
    ...
    auto& bstr = booster->at(rbtbooster);

    if (level <= 0.001 || bstr.thrust <= 0 || bstr.N <= 0) {
        return 0.0;
    }

    double m_prop = GetPropellantMass(bstr.tank);
    if (m_prop <= 0.001) return 0.0;

    double total_thrust = bstr.thrust * bstr.N * level;

    double BT = (bstr.isp * m_prop * G) / total_thrust;
...
void Multistage2026::CreateMainThruster() {
    ...
    auto& stg = stage->at(currentStage);

    if (stg.nEngines == 0) {    
        stg.nEngines = 1;
        stg.eng.at(0).x = 0;
        stg.eng.at(0).y = 0;
        stg.eng.at(0).z = -stg.height * 0.5;
    }

    if ((stg.ispref >= 0) && (stg.pref == 0)) {
        stg.pref = 101400.0;
    }

    for (int i = 0; i < stg.nEngines; i++) {
        if (i >= (int)stg.th_main_h.size()) break;

        if (Misc.thrustrealpos) {
            stg.th_main_h.at(i) = CreateThruster(stg.off, stg.eng_dir, stg.thrust / stg.nEngines, stg.tank, stg.isp, stg.ispref, stg.pref);
        } else {
            stg.th_main_h.at(i) = CreateThruster(_V(0, 0, 0), _V(0, 0, 1), stg.thrust / stg.nEngines, stg.tank, stg.isp, stg.ispref, stg.pref);
        }
    }

    thg_h_main = CreateThrusterGroup(stg.th_main_h.data(), stg.nEngines, THGROUP_MAIN);
    SetDefaultPropellantResource(stg.tank);

    SURFHANDLE hTex = GetProperExhaustTexture(stg.eng_tex);
    if (hTex == nullptr && !stg.eng_tex.empty()) {
        oapiWriteLogV("WARNING: Texture '%s' not found. Using default.", stg.eng_tex.c_str());
    }

    for (int i = 0; i < stg.nEngines; i++) {
        if (i >= (int)stg.th_main_h.size() || i >= (int)exhaustN.at(currentStage).size()) continue;

        double lsize = 10.0 * stg.eng_diameter * stg.engV4.at(i).t;
        double wsize = stg.eng_diameter * stg.engV4.at(i).t;
        ...
        exhaustN.at(currentStage).at(i) = AddExhaust(
            ...
            wsize, 
            stg.eng.at(i), 
            operator*(stg.eng_dir, -1), 
            hTex
        );

		oapiWriteLogV("MS2026 DEBUG: Exhaust created for Stage %d Engine %d (Handle: %p)", currentStage, i, exhaustN.at(currentStage).at(i));

        if (!stg.ParticlesPacked) {
            if (stg.wps1) {
                auto ps1 = GetProperPS(stg.eng_pstream1);
                AddExhaustStreamGrowing(stg.th_main_h.at(i), stg.eng.at(i), &ps1.Pss, ps1.Growing, ps1.GrowFactor_size, ps1.GrowFactor_rate, true, false, currentStage, i);
            }
            if (stg.wps2) {
                auto ps2 = GetProperPS(stg.eng_pstream2);
                AddExhaustStreamGrowing(stg.th_main_h.at(i), stg.eng.at(i), &ps2.Pss, ps2.Growing, ps2.GrowFactor_size, ps2.GrowFactor_rate, true, false, currentStage, i);
            }
        }
    }

    if (stg.ParticlesPacked) {
        int rawIdx = stg.ParticlesPackedToEngine;
        int engine = (rawIdx == 0) ? 0 : abs(rawIdx) - 1;

        if (engine >= 0 && engine < (int)stg.th_main_h.size() && engine < (int)stg.eng.size()) {
            
            std::array<PARTICLESTREAMSPEC, 2> partpacked;
            partpacked.at(0) = GetProperPS(stg.eng_pstream1).Pss;
            partpacked.at(1) = GetProperPS(stg.eng_pstream2).Pss;

            VECTOR3 Pos_stream[2];
            if (stg.ParticlesPackedToEngine > 0) {
                Pos_stream[0] = stg.eng.at(engine);
                Pos_stream[1] = stg.eng.at(engine);
            } else {
                double Posx = 0, Posy = 0, Posz = 0;
                for (int x = 0; x < stg.nEngines; x++) {
                    Posx += stg.eng.at(x).x;
                    Posy += stg.eng.at(x).y;
                    Posz += stg.eng.at(x).z;
                }
                Pos_stream[0] = _V(Posx / stg.nEngines, Posy / stg.nEngines, Posz / stg.nEngines);
                Pos_stream[1] = Pos_stream[0];
            }

            if (stg.wps1) {
                auto ps1 = GetProperPS(stg.eng_pstream1);
                AddExhaustStreamGrowing(stg.th_main_h.at(engine), Pos_stream[0], &partpacked.at(0), ps1.Growing, ps1.GrowFactor_size, ps1.GrowFactor_rate, true, false, currentStage, engine);
            }
            if (stg.wps2) {
                auto ps2 = GetProperPS(stg.eng_pstream2);
                AddExhaustStreamGrowing(stg.th_main_h.at(engine), Pos_stream[1], &partpacked.at(1), ps2.Growing, ps2.GrowFactor_size, ps2.GrowFactor_rate, true, false, currentStage, engine);
            }
        }
    }
    ...
    if (stg.DenyIgnition) {
        for (int i = 0; i < stg.nEngines; i++) {
            if (stg.th_main_h.at(i)) SetThrusterResource(stg.th_main_h.at(i), nullptr);
        }
    }

    LightEmitter* le = AddPointLight(stg.eng.at(0), 200, 1e-3, 0, 2e-3, col_d, col_s, col_a);
    if (le) le->SetIntensityRef(&th_main_level);
}
...
void Multistage2026::RotatePayload(int pns, int nm, VECTOR3 anglesrad) {
	...
	if (!DeveloperMode) {
		payloadrotatex.at(pns).at(nm) = new MGROUP_ROTATE(payload->at(pns).msh_idh.at(nm), NULL, 0, reference, _V(1, 0, 0), (float)2 * PI);
		...
		payloadrotatey.at(pns).at(nm) = new MGROUP_ROTATE(payload->at(pns).msh_idh.at(nm), NULL, 0, reference, _V(0, 1, 0), (float)2 * PI);
		...
		payloadrotatez.at(pns).at(nm) = new MGROUP_ROTATE(payload->at(pns).msh_idh.at(nm), NULL, 0, reference, _V(0, 0, 1), (float)2 * PI);
...
void Multistage2026::LoadMeshes() {
	...
	for (int pns = currentPayload; pns < nPayloads; pns++) {
		...
		live_a.at(pns) = CreateAttachment(false, payload->at(pns).off.at(0), direction, normal, "MS2026", false);
...
void Multistage2026::Guidance_Debug() {
	...
	double DesiredPitch;
	if (Gnc_step.at(step).gnc_Comand == GNC_Comand::CM_ROLL) {
	...
	else if (Gnc_step.at(step).gnc_Comand == GNC_Comand::CM_PITCH) {
...
void Multistage2026::clbkLoadStateEx(FILEHANDLE scn, void* vs){
	...
	oapiWriteLogV("DEBUG: Ruta final normalizada: %s", rawPath.c_str());
...
int Multistage2026::clbkConsumeBufferedKey(int key, bool down, char* kstate) {
	...
	if (KEYDOWN(kstate, OAPI_KEY_P) && (!KEYMOD_CONTROL(kstate)) && (!KEYMOD_SHIFT(kstate) && (!KEYMOD_ALT(kstate)))) {
		APstat = !APstat;
    	oapiWriteLogV("KEY P PRESSED: APstat is now %s", APstat ? "ON" : "OFF");
    	if (APstat) {
        	if (nsteps > 0) t_start = oapiGetSimTime() - Gnc_step.at(0).time;
	        else t_start = oapiGetSimTime();
    	}
    	return 1;
	}
...
bool Multistage2026::clbkDrawHUD(int mode, const HUDPAINTSPEC* hps, oapi::Sketchpad* skp) {
	...
	VECTOR3 time = hms(MET);

	if (MET >= 0) {
		METlen = sprintf_s(METbuff, "MET: %03.0f:%02.0f:%02.0f", time.x, time.y, time.z);
	}
	else {
		METlen = sprintf_s(METbuff, "T-: %03.0f:%02.0f:%02.0f", time.x, time.y, time.z);
	}
...
void Multistage2026::clbkPostStep(double simt, double simdt, double mjd){
	...
	if (!APstat) {
		if ((GetThrusterGroupLevel(THGROUP_MAIN) > 0.95) && (Configuration == 0)) {
			Configuration = 1;
			MET = 0; 
			oapiWriteLog(const_cast<char *>("Manual Launch Detected: MET reset to 0"));
    }
}
...
void Multistage2026::Ramp(bool alreadyramp) {
    ...
    std::string PadName = std::format("MS_LaunchPad_{}", GetName());
    oapiWriteLogV("MS2026: Initializing Launchpad %s", PadName.c_str());

    if (!alreadyramp) {
        VESSELSTATUS2 vs_temp;
        memset(&vs_temp, 0, sizeof(vs_temp));
        vs_temp.version = 2;
        GetStatusEx(&vs_temp);

        hramp = oapiCreateVesselEx(PadName.c_str(), Misc.PadModule.c_str(), &vs_temp);
        
        if (!hramp) {
            oapiWriteLogV("MS2026: ERROR: Could not create Pad vessel with module %s", Misc.PadModule.c_str());
            return; 
        }
        ...
        if (hramp) vramp = (VESSEL4*)oapiGetVesselInterface(hramp);
    }
    ...
    double z_offset = (stage->size() > 0) ? (stage->at(0).height * 0.5 + CogElev) : 10.0;

    padramp = vramp->CreateAttachment(false, _V(0, 0, z_offset), 
                                      _V(0, -sin(Misc.VerticalAngle), cos(Misc.VerticalAngle)), 
                                      _V(0, cos(Misc.VerticalAngle), sin(Misc.VerticalAngle)), 
                                      "pad", false);

    vramp->AttachChild(GetHandle(), padramp, AttToRamp);

    if (wLaunchFX) CreateLaunchFX();
